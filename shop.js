...
const renderShop = () => {
  purchases.innerHTML = cart.getAll().map((item) => `
    <div class="row d-flex justify-content-between align-items-center mb-3">
      ...
      <span class="col-5 col-xl-2 fs-6">${cart.getPriceItem(item.id) * item.count} BYN</span>
      <i id=${item.id} data-manager="removeItem" class="bi bi-x-circle col-2 col-xl-1 fs-5 text-end"></i>
    </div>
  `).join('');
